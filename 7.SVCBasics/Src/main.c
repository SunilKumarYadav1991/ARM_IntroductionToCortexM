 ...
 * @brief          : Example program to understand how to use SVCall exception work
 * and how to leverage it in embedded OS kind of environment. In RTOS or eOS
 * environment user level task should never be allowed direct access to critical
 * resources. All request should go through kernel level task. SVC instruction is
 * used to achieve this where user level task raise SVC instruction (assembly) and
 * kernel level(privileged) task decodes SVC no. and performs the task IF it find
 * eligible wrt user level task.
 *
 * In current exercise we will raise SVC instruction from thread mode and in
 * handler mode we will increment the SVC no. and return back to thread mode.
 ******************************************************************************
...
int main(void)
{
	// program star in thread mode by default. Raising SVC exception
	__asm volatile("SVC #20");              // SVC_Handler will be called

	// After call to svc handler we are returning incremented svc no. to r0 register as per ARM procedure call standard.
	uint32_t data;
	__asm volatile("MOV %0, R0" : "=r"(data) :: );     // copy data from register r0 to c variable data

	printf("Value of incremented SVC is %d\n", data);
...
__attribute__ ((naked)) void SVC_Handler(void) {
	...
	__asm ("B SVC_Handler_c");            // branch to SVC_Handler_c(r0)
}
...
void SVC_Handler_c(uint32_t *pSP) {
	...
	// increment SVC no. with 4
	svcNumber += 4;

	// writting new svc no. to register r0 which thread mode code can retrive
	pSP[0] = svcNumber;
